...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt"
      ...
      <xsl:when test="b:StyleNameLocalized">
        <xsl:choose>
          ...
          <xsl:when test="b:StyleNameLocalized/b:Lcid='1063'">
            <xsl:text>Harvardas – OU</xsl:text>
          </xsl:when>
          ...
          <xsl:when test="b:StyleNameLocalized/b:Lcid='1060'">
            <xsl:text>Harvardski OU</xsl:text>
          </xsl:when>
        </xsl:choose>
      </xsl:when>
	...
	<!-- RM 2020 -->
	<xsl:template name ="BibDisplayInternetSiteTitle">
		...
		<xsl:choose>
			<xsl:when test ="$cAuthors=0">
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:value-of select="b:Author/b:Author/b:Corporate"/>
						<!-- RM 2020 <xsl:call-template name ="templ_prop_ListSeparator"/> -->
						<xsl:call-template name ="templ_prop_Space"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
						...
						<xsl:when test="$cAuthorFirstName=1">
							...
							<xsl:choose>
								<xsl:when test ="$cAuthorMiddleName=0 and $cAuthors>1 and (position()+1)=$cAuthors">
									<xsl:call-template name ="templ_prop_Space"/>
									<!-- <xsl:call-template name ="templ_prop_BeforeLastAuthor"/>  author -->
	...
	<xsl:template name="BibDisplayYear">
		...
		<xsl:choose>
			...
			<xsl:call-template name ="templ_prop_OpenBracket"/>
				<xsl:value-of select = "b:Year"/>
			<xsl:call-template name ="templ_prop_CloseBracket"/>
				<!-- RM 2020 <xsl:call-template name = "templ_prop_Dot"/>-->
			</xsl:when>
			<xsl:otherwise>
				<xsl:call-template name ="templ_str_NoDateShortUnCap"/>
			</xsl:otherwise>
	...
	<xsl:template name="BibDisplayTitle">
		...
		<xsl:template name="BibDisplayDocTitle">
		<xsl:variable name="cTitle">
			<xsl:value-of select="count(b:Title)"/>
		</xsl:variable>
		<xsl:if test ="$cTitle!=0">
...
<!--	
			...
			<xsl:choose>
				...
				<xsl:when test="$SourceType = 'BookSection'">
					<xsl:element name="p">
						...
						<!-- RM 2020 -->
						<xsl:variable name="str_InNameUnCap">
						...
						<xsl:call-template name ="FindReplaceString">
							<xsl:with-param name="originalString" select="string($str_InNameUnCap)"/>
							<xsl:with-param name="stringToBeReplaced" select="' %1'"/>
	...
	<xsl:template name="templ_prop_SimpleQuote" >
		<xsl:text>'</xsl:text>
	</xsl:template>
